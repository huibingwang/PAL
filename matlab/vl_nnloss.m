function y = vl_nnloss(x,c,weight,flag,dzdy,varargin)
...
opts.classWeights = [] ;
opts.threshold = 0 ;
opts.rate = 0.5;  %normal label smooth VeRi：0.5; VehicleID：0.05
opts.loss = 'softmaxlog' ;
opts.gan = 0.5; %tradeoff between two loss: softmax and labelsmooth
opts.topK = 5 ;
...
switch lower(opts.loss)
    case {'log', 'softmaxlog', 'mhinge', 'mshinge','weightedlabelsmooth'}
...
if nargin <= 2 || isempty(dzdy)
    switch lower(opts.loss)
        ...
        case 'weightedlabelsmooth'
            ...
            w = size(x,4);
            tmp= sum(weight,2);
            WX=size(find(tmp~=0),1);
            ratio = size(weight,1)/WX;
            part1 = t1;  % softmax
            weight = reshape(weight,1,1,size(weight,1),size(weight,2));
            part2 = (bsxfun(@times,sum(weight/(WX+1),3),log(sum(ex,3))) -  sum(bsxfun(@times,weight/(WX+1),bsxfun(@minus,x,Xmax)),3)) * ratio/(WX+1);  % label smooth
            t = (1-opts.rate) * part1 + opts.rate *part2;

            gan = find(flag==1);  %  VeRi :1,VehicleID:0  %
            t(gan) = part1(gan);  % remove the softmax part for generated image.
        case 'mhinge'
            t = max(0, 1 - x(ci)) ;
        case 'mshinge'
            Q = x ;
    ...
    switch lower(opts.loss)
        ...
        case 'weightedlabelsmooth'
            ...
            weight = reshape(weight,1,1,size(weight,1),size(weight,2));
            part2 = bsxfun(@minus,bsxfun(@rdivide, ex, sum(ex,3)), weight/(WX+1)* ratio/(WX));
            y = (1-opts.rate) * part1 + opts.rate *part2;  % When we set opt.rate=0, it equals to softmax loss
            gan = find(flag==1); %  VeRi :1,VehicleID:0  %
            y(:,:,:,gan) = part1(:,:,:,gan); % We set opts.gan=1
            y = bsxfun(@times, dzdy, y) ;
        case 'mhinge'
            y = zerosLike(x) ;
            y(ci) = - dzdy .* (x(ci) < 1) ;
