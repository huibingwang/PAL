...
%% add necessary paths
query_dir = '/home/datasets/VeRi/image_query/';% query directory
test_dir = '/home/datasets/VeRi/image_test/';% database directory
a = 0.9;

%% calculate query features
Hist_query = importdata('../features/query_paper_veri_iter_6.mat')';
nQuery = size(Hist_query, 2);

%% calculate database features
Hist_test = importdata('../features/test_paper_veri_6.mat')';
...
if ~exist('data/testID.mat')
    for n = 1:length(test_files)
        ...
        if strcmp(img_name(1), '-') % junk images
            ...
            testCAM(n) = str2num(img_name(8:9));
        end
    end
...
if ~exist('data/queryID.mat')
    for n = 1:length(query_files)
        ...
        if strcmp(img_name(1), '-') % junk images
            ...
            queryCAM(n) = str2num(img_name(8:9));
        end
    end
...
CMC = mean(CMC);
%% print result
fprintf('single query:                                    mAP = %f, r1 precision = %f, r5 precision = %f, r10 precision = %f, r20 precision = %f\r\n', mean(ap), CMC(1), CMC(5), CMC(10), CMC(20));
